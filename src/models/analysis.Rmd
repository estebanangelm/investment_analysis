...
```
```{r}
fundamentals <- read_csv(file ='../../data/original/fundamentals.csv')
prices <- read_csv(file ='../../data/original/prices.csv')
securities <- read_csv(file ='../../data/original/securities.csv')
```
...
```{r message=FALSE,warning=FALSE}
ggplot(var_price_revenue)+
  geom_point(aes(x=var_revenue,y=var_price),alpha=0.1,color="#4B86B4")+
  coord_cartesian(xlim = c(-0.5,0.5))+
  theme_minimal()
```
